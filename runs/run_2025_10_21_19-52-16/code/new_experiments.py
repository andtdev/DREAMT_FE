...
# Note: N2 and N3 are merged into a single "N2+N3" class
...
print(f"Note: N2+N3 class combines both original N2 and N3 stages\n")
...
# Apply scaling to minority classes (reduced from 4.0x to 2.0x to reduce instability)
scaling_factors = {
...
# Skip SMOTE - rely purely on class weights to prevent synthetic sample issues
print("\nSkipping SMOTE - using class weights only to prevent synthetic sample issues...")
print("Rationale: SMOTE-generated synthetic samples may not capture true physiological patterns,")
print("especially for REM sleep which requires specific eye movement and muscle tone characteristics.")
...
print(f"  - Class weight scaling: 2x for R and N1 (reduced from 4x)")
print(f"  - Regularization: LOW (reg_alpha 0-10, reg_lambda 0.1-1.0)")
print(f"  - Data: NO SMOTE (using original data only)")
