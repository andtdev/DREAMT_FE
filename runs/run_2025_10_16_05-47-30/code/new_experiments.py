...
# Train LightGBM multiclass model with FOCAL LOSS
print("\nTraining LightGBM multiclass model with FOCAL LOSS...")
print("Focal loss focuses training on hard-to-classify examples (minority classes)")
print("Using class weights AND focal loss for maximum minority class attention...")
final_lgb_model = LightGBM_engine_multiclass(
    ...
    focal_gamma=2.0   # Focusing parameter (2.0 = moderate, 5.0 = very aggressive)
)
...
print(f"\nModel Settings:")
print(f"  - Loss function: FOCAL LOSS (gamma=2.0, alpha=1.0)")
print(f"  - Class weights: Aggressive (N3 weight=30.88x)")
print(f"  - Regularization: reg_alpha 0-20, reg_lambda 0.01-1.0 (reduced)")
print(f"  - Focus: Hard-to-classify minority classes (R, N1, N3)")
print(f"{'='*80}")
