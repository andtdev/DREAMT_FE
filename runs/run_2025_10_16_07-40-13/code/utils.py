...
def compute_probabilities_multiclass(list_sids, df, features_list, final_model, group_variable, num_classes=5):
    """
    Computes the probabilities for multiclass classification.
    ...
    features_list : list
        A list of strings representing the names of the features to be used in the model.
    final_model: 
        The trained LightGBM model object to use for predictions.
    group_variable: list
        A list containing the selected variable(s).
    num_classes : int
        Number of classes in multiclass classification (default: 5)

    Returns:
    ----------
    list_probabilities_subject : list
        A list of the predicted probabilities (shape: [n_samples, num_classes]) for each subject.
    lengths : list
        A list of integers representing the number of predictions for each subject.
    list_true_stages : list
        A list of the true sleep stages for each subject.
    ...
    for sid in list_sids:
        ...
        pred_proba = final_model.predict_proba(x)  # Shape: [n_samples, num_classes]

        list_probabilities_subject.append(pred_proba)
        lengths.append(pred_proba.shape[0])
        list_true_stages.append(sid_df.Sleep_Stage.to_numpy())
...
def plot_cm(list_probabilities_subject, list_true_stages, model_name, image_output_file=None):
    """
    ...
    The function does not return any value. It generates and displays a confusion matrix
        heatmap plot directly or saves it to a file.
    """
    ...
    if image_output_file:
        plt.savefig(image_output_file, bbox_inches='tight', dpi=150)
        plt.close()
    else:
        plt.show()


def plot_cm_multiclass(list_probabilities_subject, list_true_stages, model_name, class_names, image_output_file=None):
    """
    ...
    model_name : str
        The name of the model to be shown on the plot.
    class_names : list
        List of class names in order (e.g., ['W', 'R', 'N1', 'N2', 'N3'])
    image_output_file : str, optional
        If provided, save the plot to this file path instead of displaying it.
